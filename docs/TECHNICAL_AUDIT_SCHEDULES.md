...
| 9 | Import replacement safety | ✅ **Complete** | Removed replacement deletion behavior. Imports are upsert-only: empty cells preserve existing schedules, explicit `Off` cells create no schedule record, and today/past rows are skipped and reported. |
...
- **Item 9 — Import replacement safety:** complete. Imports are upsert-only, empty cells preserve existing schedules, `Off` creates no schedule record, and protected rows are skipped and reported.
