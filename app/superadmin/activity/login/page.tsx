...
function ActivityRow({ activity }: { activity: LoginActivityRecord }) {
  ...
  return <div className="flex flex-col gap-3 rounded-lg border border-border bg-background p-4 md:flex-row md:items-center md:justify-between">
    ...
    <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><TooltipProvider><Tooltip><TooltipTrigger asChild><button type="button" className="font-medium text-foreground underline decoration-dotted underline-offset-4">{activity.channel}</button></TooltipTrigger><TooltipContent><p>IP address: {activity.ipAddress}</p><p className="mt-1 max-w-md break-all">Device: {activity.device}</p></TooltipContent></Tooltip><span aria-hidden="true">·</span><Tooltip><TooltipTrigger asChild><button type="button" className={activity.result === 'SUCCESS' ? 'font-medium text-foreground underline decoration-dotted underline-offset-4' : 'font-medium text-destructive underline decoration-dotted underline-offset-4'}>{shortReasonFor(activity.result)}</button></TooltipTrigger><TooltipContent><p>Reason: {reasonFor(activity.result)}</p></TooltipContent></Tooltip></TooltipProvider></div>
  </div>
}
