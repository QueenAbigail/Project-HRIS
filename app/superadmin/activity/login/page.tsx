...
function ActivityRow({ activity }: { activity: LoginActivityRecord }) {
  ...
  return <div className="flex flex-col gap-3 rounded-lg border border-border bg-background p-4 md:flex-row md:items-center md:justify-between">
    <div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate font-medium">{activity.employeeName}</p><Badge variant="outline" className="shrink-0 text-[10px]">{activity.employeeId}</Badge>{activity.isDummy && <Badge variant="outline" className="text-[10px]">DEMO</Badge>}</div><p className="truncate text-xs text-muted-foreground">{activity.email}</p><div className="mt-2 flex flex-wrap gap-1.5"><Badge variant="outline" className="text-[11px]">{date}</Badge><Badge variant="outline" className="text-[11px]">{time}</Badge></div></div>
