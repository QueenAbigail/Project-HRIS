  ...
  return <Card className="flex h-full min-h-[500px] flex-col border border-border bg-card">
    ...
    <CardContent className="flex-1 pt-6"><ScrollArea className="h-full pr-4"><div className="space-y-3">
      {activities.map((activity) => { const [date, time] = activity.timestamp.split(' '); return <div key={activity.id} className="rounded-lg border border-border bg-background p-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><TooltipProvider><Tooltip><TooltipTrigger asChild><button type="button" className="font-medium text-foreground">{activity.channel}</button></TooltipTrigger><TooltipContent><p>IP address: {activity.ipAddress}</p><p className="mt-1 max-w-sm break-all">Device: {activity.device}</p></TooltipContent></Tooltip><span aria-hidden="true">·</span><Tooltip><TooltipTrigger asChild><button type="button" className={activity.result === 'SUCCESS' ? 'font-medium text-foreground' : 'font-medium text-destructive'}>{shortReasonFor(activity.result)}</button></TooltipTrigger><TooltipContent><p className="max-w-sm">Reason: {reasonFor(activity.result)}</p></TooltipContent></Tooltip></TooltipProvider></div>
      </div> })}
