  ...
  return <Card className="flex h-full min-h-[500px] flex-col border border-border bg-card">
    ...
    <CardContent className="flex-1 pt-6"><ScrollArea className="h-full pr-4"><div className="space-y-3">
      {activities.map((activity) => { const [date, time] = activity.timestamp.split(' '); return <div key={activity.id} className="rounded-lg border border-border bg-background p-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5"><Badge variant="outline" className="text-[11px]">{activity.channel}</Badge><Badge variant="outline" className="text-[11px]">{activity.ipAddress}</Badge><TooltipProvider><Tooltip><TooltipTrigger asChild><Badge variant="secondary" className="max-w-[150px] cursor-help truncate text-[11px]">{shortDevice(activity.device)}</Badge></TooltipTrigger><TooltipContent><p className="max-w-sm break-all">{activity.device}</p></TooltipContent></Tooltip></TooltipProvider></div>
