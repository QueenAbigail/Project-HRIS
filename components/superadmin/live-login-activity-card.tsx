  ...
  return <Card className="flex h-full min-h-[500px] flex-col border border-border bg-card">
    ...
    <CardContent className="flex-1 pt-6"><ScrollArea className="h-full pr-4"><div className="space-y-3">
      {activities.map((activity) => { const [date, time] = activity.timestamp.split(' '); return <div key={activity.id} className="rounded-lg border border-border bg-background p-3">
        <div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="flex min-w-0 items-center gap-1.5"><p className="truncate text-sm font-medium">{activity.employeeName}</p><span className="shrink-0 text-xs text-muted-foreground">· {activity.employeeId}</span>{activity.isDummy && <span className="shrink-0 text-[10px] uppercase tracking-wide text-muted-foreground">(demo)</span>}</div><div className="mt-1 flex flex-wrap gap-1"><Badge variant="outline" className="text-[11px]">{date}</Badge><Badge variant="outline" className="text-[11px]">{time}</Badge></div></div><Badge variant={activity.result === 'SUCCESS' ? 'secondary' : 'destructive'} className="shrink-0 text-[11px]">{activity.result === 'SUCCESS' ? 'SUCCESS' : 'FAILED'}</Badge></div>
        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><TooltipProvider><Tooltip><TooltipTrigger asChild><button type="button" className="font-medium text-foreground underline decoration-dotted underline-offset-4">{activity.channel}</button></TooltipTrigger><TooltipContent><p>IP address: {activity.ipAddress}</p><p className="mt-1 max-w-sm break-all">Device: {activity.device}</p></TooltipContent></Tooltip><span aria-hidden="true">·</span><Tooltip><TooltipTrigger asChild><button type="button" className={activity.result === 'SUCCESS' ? 'font-medium text-foreground underline decoration-dotted underline-offset-4' : 'font-medium text-destructive underline decoration-dotted underline-offset-4'}>{shortReasonFor(activity.result)}</button></TooltipTrigger><TooltipContent><p className="max-w-sm">Reason: {reasonFor(activity.result)}</p></TooltipContent></Tooltip></TooltipProvider></div>
      </div> })}
